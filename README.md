...
## 4️⃣0️⃣ 💼 Personal Portfolio Website with HTML
- **설명:**
이 프로젝트는 HTML을 이용해 만든 개인 소개 웹페이지입니다.
자신의 소개, 학력, 경력, 기술 스택, 그리고 취미 및 연락처 페이지를 구성하여,
하나의 깔끔한 이력서형 웹사이트로 만들었습니다.

- **특징:**
  - <table>을 사용해 프로필/소개, 경력, 스킬을 구조화
...
- **설명:**
이 프로젝트는 이전 HTML 기반 포트폴리오에 CSS를 연동하여 시각적으로 개선한 버전입니다.
외부 스타일시트(styles.css)를 통해 배경색, 이미지 스타일, 구분선, 텍스트 색상 등을 지정하여
더 깔끔하고 정돈된 개인 웹페이지를 구현했습니다.

- **특징:**
  - 외부 CSS 파일(styles.css) 연동을 통한 스타일 분리 및 재사용성 향상
  - 프로필 이미지를 원형으로 만들고, 배경 색상 및 테두리 스타일 지정
